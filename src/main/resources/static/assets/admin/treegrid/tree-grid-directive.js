        ...
        .run([
            ...
            function ($templateCache) {
                $templateCache.put('template/treeGrid/treeGrid.html',
                    ...
                    "         <div ng-if=\"!col.cellTemplate\">{{row.branch[col.field]?'Active':'Not active'}}</div>\n" +
                    "       </td>\n" +
                    "       <td>\n" +
                    "         <div class=\"btn-group btn-group-xs\">\n" +
                    "           <button ng-click=\"edit({item:row.branch})\" class=\"btn btn-warning\">\n" +
                    "            <i class=\"material-icons\">mode_edit</i></button>\n" +
                    "           <button ng-click=\"delete({item:row.branch})\" class=\"btn btn-danger\">\n" +
            ...
            function ($timeout,
                ...
                return {
                    ...
                    link: function (scope, element, attrs) {
                        ...
                        attrs.iconExpand = attrs.iconExpand ? attrs.iconExpand : 'icon-chevron-right  glyphicon glyphicon-chevron-right  fa fa-chevron-right';
                        attrs.iconCollapse = attrs.iconCollapse ? attrs.iconCollapse : 'icon-chevron-down glyphicon glyphicon-chevron-down fa fa-chevron-down';
                        ...
                        if (!attrs.colDefs) {
                            if (scope.treeData.length) {
                                var _col_defs = [],
                                    ...
                                    _unwantedColumn = ['children', 'level', 'expanded', 'icons','id','parent_id', expandingProperty];
                                for (var idx in _firstRow) {
                                    if (_unwantedColumn.indexOf(idx) === -1) {
                                        _col_defs.push({
                                            field: idx,
                                            displayName: idx.charAt(0).toUpperCase() + idx.slice(1)
                                        });
                                    }
                                }
